...
exports.submitLeave = async (req, res) => {
  ...
  try {
    ...
    if (existingLeave) {
      return res
        .status(400)
        .json({ message: "You have already applied for leave today." });
    }
    ...
    const leave = new Leave({
      ...
      coordinatorId,
      department,
      // Assign the selected coordinator
    });
  ...
  <html>
    <body style="font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <p>An early leave application has been submitted by <strong>${
        user.name
      }</strong> (Enrollment Number: <strong>${
      user.enrollmentNumber
    }</strong>) from <strong>Class ${user.class}, Year ${
      user.year
    }</strong>.</p>
...
`;
    ...
    res
      .status(400)
      .json({ message: "Leave submission failed", error: err.message });
  }
};
  ...
  try {
    ...
    // HOD approval
      ...
      if (leave.coordinatorApprovalStatus === "approved") {
        ...
        if (leave.finalStatus !== "pending") {
          ...
          const emailHtml = `
            <div style="font-family: Arial, sans-serif; color: #333; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
              ...
              <p>Your leave request from <strong>${leave.startDate.toDateString()}</strong> to <strong>${leave.endDate.toDateString()}</strong> has been <span style="color: ${
            leave.finalStatus === "approved" ? "#28a745" : "#dc3545"
          }; font-weight: bold;">${leave.finalStatus}</span>.</p>
              <p><strong>Reason for Leave:</strong> ${leave.reason}</p>
        ...
        return res
          .status(403)
          .json({ message: "Coordinator approval is required first" });
      }
    } else {
      return res
        .status(403)
        .json({ message: "Not authorized to update this leave status" });
    }
...
exports.getHODLeaves = async (req, res) => {
  try {
    const { id } = req.user;
    const findHOD = await User.findById(id);
    const { department } = findHOD;
    ...
    res
      .status(400)
      .json({ message: "Failed to fetch leaves for HOD", error: err.message });
  }
};
...
exports.getCoordinatorLeaves = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "Failed to fetch leaves for coordinator",
      error: err.message,
    });
  }
};
...
exports.getLeaveById = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Error fetching leave details", error: err.message });
  }
};
...
exports.getLeavesByDate = async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ message: "Error fetching leaves by date", error: err.message });
  }
};
...
exports.getLeaveHistory = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Failed to fetch leave history", error: err.message });
  }
};
...
exports.getLeaveStats = async (req, res) => {
  try {
    ...
    const totalLeaves =
      rejected.length + approvedLeaves.length + pendingLeaves.length;
...
exports.getAllApprovedLeaves = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Failed to fetch approved leaves", error: err.message });
  }
};
